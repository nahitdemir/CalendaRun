...
public class AuditLogsController : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            return BadRequest(Calendarun.Common.Errors.ProblemDetailsFactory.Create(400, adminFromError ?? "Invalid date format", HttpContext));
        }

        if (!DateQueryParser.TryParseDateFilter(to, "to", true, out var adminDateTo, out var adminToError))
        {
            return BadRequest(Calendarun.Common.Errors.ProblemDetailsFactory.Create(400, adminToError ?? "Invalid date format", HttpContext));
        }
